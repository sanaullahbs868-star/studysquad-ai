...
export class CommandManager {
  ...
  executeCommand(player: Player, command: string): CommandResult {
    // Only admins and owners can use commands
    if (player.role !== "admin" && player.role !== "owner") {
      return {
        success: false,
        message: "❌ Only admins and owners can use commands",
      };
    }
  ...
  private flyCommand(player: Player): CommandResult {
    ...
    return {
      success: true,
      message: `✈️ ${player.username} is flying! Height: ${newPosition.y}`,
      data: newPosition,
    };
  }
  ...
  private giveCommand(player: Player, args: string[]): CommandResult {
    ...
    if (!playerName || !itemName) {
      return {
        success: false,
        message: "❌ Usage: /give <player_name> <item_name>",
      };
    }
    ...
    return {
      success: true,
      message: `📦 Gave ${itemName} to ${playerName}`,
    };
  }
  ...
  private muteCommand(args: string[]): CommandResult {
    ...
    if (!playerName) {
      return {
        success: false,
        message: "❌ Usage: /mute <player_name>",
      };
    }
    ...
    return {
      success: true,
      message: `🔇 ${playerName} has been muted`,
    };
  }
  ...
  private unmuteCommand(args: string[]): CommandResult {
    ...
    if (!playerName) {
      return {
        success: false,
        message: "❌ Usage: /unmute <player_name>",
      };
    }
    ...
    return {
      success: true,
      message: `🔊 ${playerName} has been unmuted`,
    };
  }
  ...
  private promoteCommand(args: string[]): CommandResult {
    ...
    if (!playerName || !newRole) {
      return {
        success: false,
        message:
          "❌ Usage: /promote <player_name> <role:admin|owner|developer|beginner>",
      };
    }
    ...
    return {
      success: true,
      message: `⬆️ ${playerName} promoted to ${newRole}`,
    };
  }
  ...
  private demoteCommand(args: string[]): CommandResult {
    ...
    if (!playerName) {
      return {
        success: false,
        message: "❌ Usage: /demote <player_name>",
      };
    }
    ...
    return {
      success: true,
      message: `⬇️ ${playerName} demoted to beginner`,
    };
  }
  ...
  private kickCommand(args: string[]): CommandResult {
    ...
    if (!playerName) {
      return {
        success: false,
        message: "❌ Usage: /kick <player_name>",
      };
    }
    ...
    return {
      success: true,
      message: `👋 ${playerName} has been kicked from the server`,
    };
  }
  ...
  private setXPCommand(args: string[]): CommandResult {
    ...
    if (!playerName || !xpStr) {
      return {
        success: false,
        message: "❌ Usage: /setxp <player_name> <xp_amount>",
      };
    }
    ...
    if (isNaN(xpAmount)) {
      return {
        success: false,
        message: "❌ XP amount must be a number",
      };
    }
    ...
    return {
      success: true,
      message: `⭐ Set ${playerName}'s XP to ${xpAmount} (Level: ${player.level})`,
    };
  }
  ...
  private addMoneyCommand(args: string[]): CommandResult {
    ...
    if (!playerName || !moneyStr) {
      return {
        success: false,
        message: "❌ Usage: /addmoney <player_name> <amount>",
      };
    }
    ...
    return {
      success: true,
      message: `💰 Added $${amount} to ${playerName} (New balance: $${player.money})`,
    };
  }
...
📋 **ADMIN COMMANDS** (Admin/Owner only):
  ...
  /promote <player> <role> - Promote player (admin|owner|developer|beginner)
  ...
  /addmoney <player> <amount> - Give money to player
